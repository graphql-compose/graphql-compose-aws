  ...
  getArgs(): GraphQLFieldConfigArgumentMap {
    if (!this.args) {
      if (!this.config.input) {
        this.args = {} as GraphQLFieldConfigArgumentMap;
      } else {
        ...
        if (Object.keys(itc.getFields()).length === 0) {
          ...
          const hasRequiredFields = itc.getFieldNames().some((f) => itc.isFieldNonNull(f));
          type = hasRequiredFields ? itc.getTypeNonNull().getType() : itc.getType();
        }
  ...
  getResolve(): GraphQLFieldResolver<any, any> {
    return (source: any, args: { [argument: string]: any }) => {
      return new Promise((resolve, reject) => {
        ...
        service[this.name](args.input, (err: any, data: any) => {
          if (err) {
            reject(err);
          }
          resolve(data);
