...
import { AwsServiceMetadata, ServiceMetadataConfig } from './AwsServiceMetadata';
import { AwsServiceOperation, ServiceOperationConfig } from './AwsServiceOperation';
import { AwsShapes, ShapesMap } from './AwsShapes';
...
export class AwsService<TContext> {
  ...
  static lowerFirst(name: string): string {
  ...
  getOperation(name: string): AwsServiceOperation<TContext> {
    ...
    return new AwsServiceOperation({
      ...
      name: AwsService.lowerFirst(name),
      prefix: this.prefix,
      config: operConfig,
      shapes: this.shapes,
      awsSDK: this.awsSDK,
  ...
  getTypeComposer(): ObjectTypeComposer<any, TContext> {
    if (!this.tc) {
      const fields = this.getOperationNames().reduce((res, name) => {
        res[AwsService.lowerFirst(name)] = this.getOperation(name).getFieldConfig();
        return res;
      }, {} as any);
